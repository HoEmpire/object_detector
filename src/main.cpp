  ...
  else if (config.detection_mode == "lidar_first")
  {
    ...
    for (const auto &result : results)
    {
      if (result.type == 0)
      {
        ...
        int y_center = -(result.bbox.x + 0.5 * result.bbox.width - 0.5 * 1440);
        int z_center = -(result.bbox.y + 0.5 * result.bbox.height - 0.5 * 1080);

        float ratio = z_center / y_center;
        float distance_min = 1000000;
        float distance_tmp = 1000000;
        int index = -1;
        for (uint i = 0; i < markers.size(); i++)
        {
          // if (markers[i].points[0].z * z_center > 0 && markers[i].points[0].y * y_center > 0)
          // {
          float ratio_points = markers[i].points[0].z / markers[i].points[0].y;
          distance_tmp = sqrt((ratio_points - ratio) * (ratio_points - ratio));
